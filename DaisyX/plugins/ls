...
@UserBot.on_message(filters.me & filters.text & command("ls"))
async def list_dir(client, message):
    cmd = message.command
    if len(cmd) > 1:
        try:
            directory_param = cmd[1]
            directory = UserBot.main_directory + "/DaisyX" + "/" + directory_param# + f"/{directory}"
...
@UserBot.on_message(filters.me & filters.text & command("del"))
async def download(client, message):
    cmd = message.command
    if len(cmd) > 1:
        try:
            file_param = cmd[1]
            file_to_del = UserBot.main_directory + "/DaisyX" + "/" + file_param# + f"/{directory}"
            try:
...
@UserBot.on_message(filters.me & filters.text & command("send"))
async def send_files(client, message):
    ...
    if len(cmd) > 1:
        try:
            ...
            file_to_upload = str(UserBot.WORKDIR) + "/DaisyX" + "/" + file_param# + f"/{directory}"
            try:
